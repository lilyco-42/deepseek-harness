...
| 能力 | 当前证据 | 验收状态 |
|---|---|---|
| 普通问候与纠正上一轮 | `new-api` commit `4f9032e` 的 CI run `36263767947` 与服务包 run `36263767975` 通过并部署为 `agent-flat-4f9032e`。线上默认模型实测“你好”→“刚才不是只问了个问好”，能承认没接好并继续询问，Radxa 离线且未调用工具；单轮耗时 30.16 秒 | 单场景语义通过；整体仍未通过，需做多模型/重复回归并解决延迟长尾 |
