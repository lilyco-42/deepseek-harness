...
`cordis.patch.yml` 通过官方 `llm-pi-ai` 适配器，将默认模型路由到现有的 OpenAI 兼容 Lain42 API。模型 id 和 API key 来自节点所有者的环境变量，不会提交到仓库。该配置会禁用官方 DeepSeek 模型与搜索适配器，以及内置的 session-log 和 telemetry 导出。公开 URL 抓取仍可用。搜索需要另行配置提供方，因此此阶段**尚未就绪**。Actions smoke 使用模拟网关和代理提供的测试页面，验证抓取工具之后的模型请求会收到页面正文；它不验证真实 Lain42 网关是否可用或回答质量。
