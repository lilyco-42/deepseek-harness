...
`initialize` is the runtime-readiness boundary: when the server is mounted by a Loader composition, it waits for the current plugin tree to settle before replying, so async sibling capabilities such as initial MCP tool discovery are visible to the first prompt. The handshake returns the wire-stable identity `deepseek-harness-sdk-runtime`. The server validates the provider/model route and optional non-empty `reasoningEffort` through the selected adapter before it stores them; omission stores no effort, so the model retains its own default. An optional positive `maxTokens` becomes the request output cap of each SDK-created agent and its in-process descendants, while omission applies the selected adapter or provider route default. JSON-RPC requests may dispatch concurrently, so `session/prompt` rejects until one `initialize` has completed successfully; clients must await the handshake before sending prompts. An accepted prompt queues one identified user message and immediately returns `{ messageId }`; the server then streams every durable fact as `session.event` and every whole-agent lifecycle transition as `session.status`. `session/cancel` asks one open agent to stop its current work and returns before that activity settles. `session/close` disposes one live agent but leaves durable session data and the runtime available. When the composition provides `sessionPersistence`, the next prompt for that id resumes the stored history; without persistence, it creates a fresh session. The server does not assign an assistant message or `turn/end` to a prompt, and independent requests may enqueue more work on the same session. Persistence roots and persona come from the surrounding composition.

When an SDK-owned agent needs a permission decision, the server sends `approval/request` to the host and waits for its one-shot answer. The payload contains only the session id, tool name, optional call id, and reason; a tool's arguments and credentials are not copied. A valid `allowed-once` is the only grant. Missing or malformed answers fail closed as `unavailable`. Cancellation propagates through `$/cancelRequest`, allowing the host to dismiss its pending approval UI.
...
Each protocol method validates its inputs and resolves the owning state before acting — `initialize` stores the SDK route, `session/prompt` resolves the live agent+session pair and queues the message, `session/cancel` requests cancellation on one open agent, `session/close` disposes one live agent, and `shutdown` flushes the response before disposing the root context to quiescence and exiting 0. A shared exit task guarantees that racing `shutdown` requests never dispose or exit twice. The dispatch lives in [src/index.ts](src/index.ts) and [src/server.ts](src/server.ts).
...
- **There is no per-prompt result** — `messageId` identifies inbox admission only; clients observe session events and status to determine how activity settled.
