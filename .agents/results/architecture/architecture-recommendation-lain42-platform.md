...
Status: Proposed — 2026-09-26；基于 2026-09-27 源码审计修订。本文是目标架构与迁移建议，不是已上线功能清单。
...
本次源码审计确认，DSH 的 SessionController 已提供完整会话命令和事件流，但这不构成 Lain42 的多租户授权层：

- SessionController 创建请求可带 workspaceId、cwd 和 sessionId；多个方法再以 sessionId 查询会话或主机能力。直接把整组 Remote API 暴露给网页，会让浏览器请求触达 DSH Host 的工作区和本地能力边界。
- DSH Gateway 的 PeerScope 表示已接入 DSH Connection 的传输调用方作用域；目前没有证据表明它自动映射到 New API 已登录账户或 GitHub OAuth 归属。
- New API 网站 Agent 工具路由已有 UserAuth 中间件，控制器以服务端认证身份获取用户 ID；OAuth 凭据访问和工具请求因此可以继续留在 New API 业务边界内。
- 当前生产聊天仍由 New API 网站路径承载；没有发现 New API 到 DSH 的已部署会话/模型桥。新增 OAuth 错误反馈 commit 4ce049d 的 Actions 与服务包构建均通过，但线上状态仍是 agent-4f9032e，不能作为已部署或已完成的 DSH 闭环证据。

据此收紧推荐实现：浏览器只访问 New API 的登录态产品接口；New API 服务端负责身份、权益、凭据与权限校验，再通过仅内网可达、短时签名且限定能力的服务调用连接 DSH。DSH 只接收经验证的主体和会话范围，并把主体归属写入会话、事件与持久对象。初版 DSH 站点能力采用 web-safe allowlist：不传递任意 cwd/workspace，不挂载共享主机文件系统，不开放原生文件操作、shell、管理员配置或任意插件安装。需要读写项目时，由 DSH 委派到该用户自己明确授权的设备执行器。

如果现有 DSH 进程/Remote 组合无法在所有入口可靠执行上述授权，则应在服务端增加 Lain42 会话 Facade，仅映射产品确实需要的少量方法，不把 DSH Gateway 直接挂到公开网站。优先验证 New API → DSH 的连接数、空闲资源和会话恢复，再决定同机进程或内网服务部署；不按框图预先拆成更多常驻服务。

该补充把 P0 的第一项改为：先完成主体/会话归属与 web-safe 能力契约及反例测试，再开发隐藏 DSH 测试入口。账户隔离、附件内容确认和真实工具后续回答仍是整个功能放行条件。
