...
DeepSeek Harness（DSH）继续作为 Web 与移动端产品运行时，负责界面、身份、会话、模型路由和工具策略。ZeroStack 只作为可选外部 ACP worker，通过现有 [`subagent-acp` 提供方](../../../../packages/subagent/subagent-acp/README.zh.md)启动；在审查分发义务前，不把它的 Rust 代码链接进 DSH，也不捆绑其 GPL-3.0-only 二进制。
...
RSS 样本是短时空闲测量，会受到 runner 和操作系统差异影响。除非 DSH 与 ZeroStack 在同一 runner 上执行同一代表性任务，否则无法据此确认端到端内存或 CPU 节省。实现完整的跨平台沙箱和符合 GPL 的分发方式，所需工程投入可能高于当前测出的收益。
