...
const roots: string[] = []
// npm startup is measurably slower under Windows coverage instrumentation.
// These tests verify local-registry behavior, not an npm speed threshold.
const npmResolutionTestTimeoutMs = 60_000
...
describe('npm resolution benchmark', () => {
  ...
  it('runs npm against the local registry without requesting an archive', async () => {
    ...
    const result = await benchmarkNpmResolution(index, '0.1.0', npmResolutionTestTimeoutMs)
  ...
  it('isolates peer resolution from inherited npm configuration', async () => {
    ...
    try {
      ...
      const result = await resolveNpmPackageLock(index, { '@deepseek-ai/dsh': '0.1.0' }, npmResolutionTestTimeoutMs)
